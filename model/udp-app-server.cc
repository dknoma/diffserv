...
UdpAppServer::UdpAppServer ()
{
  ...
  std::chrono::system_clock::time_point m_first_total;
  std::chrono::system_clock::time_point m_first_high;
  std::chrono::system_clock::time_point m_last_high;
    std::chrono::system_clock::time_point m_first_low;
  std::chrono::system_clock::time_point m_last_low;
  std::chrono::system_clock::time_point m_last_total;
  m_received_l = 0;
  m_received_h = 0;
  m_duration_l = 0;
  m_duration_h = 0;
      ...
      if (m_received_l == 1) 
        {
          ...
          m_first_low = high_resolution_clock::now(); // Start clock when get first packet
          m_first_total = high_resolution_clock::now();
          /* TODO: Replace this for bug fix where m_count -1 packets only being received */
        }
      else if (m_received_l < m_count)
        {
           m_last_low = high_resolution_clock::now();
        }
      else if (m_received_l == m_count)
        {
          m_last_low = high_resolution_clock::now();
          ...
          typedef std::chrono::milliseconds ms;
          typedef std::chrono::duration<float> fsec;
          fsec fs = m_last_low - m_first_low;
          auto c = std::chrono::duration_cast<ms>(fs);
          m_duration_l = c.count();
          std::cout << "time is " << m_duration_l << "\n";
      ...
      if(m_received_h == 1) 
        {
          ...
          m_first_high = high_resolution_clock::now(); // Start clock when get first packet
        }
      else if (m_received_h < m_count)
        {
           m_last_high = high_resolution_clock::now();
        }
      else if(m_received_h == m_count)
        {
          /* Do the low calc here and see if that fixes it */
          typedef std::chrono::milliseconds ms;
          typedef std::chrono::duration<float> fsec;
          fsec fsLow = m_last_low - m_first_low;
          auto c = std::chrono::duration_cast<ms>(fsLow);
          m_duration_l = c.count();
          std::cout << "time is " << m_duration_l << "\n";

          m_last_high = high_resolution_clock::now();
          m_last_total = high_resolution_clock::now();
          std::cout << "Got last high entropy packet " << m_received_h << "\n";
          typedef std::chrono::milliseconds ms;
          typedef std::chrono::duration<float> fsec;
          fsec fs = m_last_high - m_first_high;
          fsec fs2 = m_last_total - m_first_total;
          auto d = std::chrono::duration_cast<ms>(fs);
          auto d2 = std::chrono::duration_cast<ms>(fs2);
          m_duration_h = d.count();   
          m_duration_total = d2.count();          
        }
    }
